...
router.post('/register', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const agentRef = await db.collection('agents').add({
            ...
            metadata: { email: body.email }, createdAt: new Date().toISOString(),
        });

        await db.collection('tokenBalances').doc(agentRef.id).set({
...
router.get('/me', authMiddleware(), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        res.json({
            ...
            email: agent.metadata?.email || '', metadata: agent.metadata || {},
