...
function validateTwilioConfig(res: Response): boolean {
    const log = getLogger('calls');
...
function validateLiveAIConfig(res: Response): boolean {
    const log = getLogger('calls');
...
async function placeRecordedCall(to: string, twiml: string) {
    const log = getLogger('calls');
    const { accountSid, authToken, fromNumber } = getTwilioConfig();
...
async function placeLiveAICall(to: string, callDocId: string) {
    const log = getLogger('calls');
    const { accountSid, authToken, fromNumber } = getTwilioConfig();
...
router.post('/call', authMiddleware(), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        if (!validateLiveAIConfig(res)) return;
        ...
        if (CALL_MODE === 'LIVE_AI_AGENT') {
            ...
            const { response: twilioResponse, data: twilioData } = await placeLiveAICall(body.to_number, callRef.id);
            ...
            const { response: twilioResponse, data: twilioData } = await placeRecordedCall(body.to_number, twiml);
